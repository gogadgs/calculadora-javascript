...
function chart(principal, interest, Monthly, Payments) {
    ...
    g.textAlign = "auto";
    var y = amountToY(0);
    for (var monthly = 1; monthly <= Payments; monthly++) {
        var x = paymentToX(monthly * 24);
        g.fillRect(x - 0, y - 3, 3, 3);
        if (monthly == 1) g.fillText("meses", x, y);
        if (monthly % 12 == 0 && monthly * 12 !== Payments)
            g.fillText(String(monthly), x, y - 10);
    };
    g.textAlign = "right";
